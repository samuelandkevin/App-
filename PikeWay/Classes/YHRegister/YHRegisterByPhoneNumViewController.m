...
- (void)requestRegisterComplete:(NetManagerCallback)complete{
    //手机号,密码，验证码必需
    //MD5加密
    NSString *codeMD5 = [YHUtils md5HexDigest:self.tfCode.text];
    [[YHNetManager sharedInstance] postRegisterWithPhoneNum:self.tfPhoneNum.text veriCode:self.tfVerifyCode.text passwd:codeMD5 complete:^(BOOL success, id obj) {
        complete(success,obj);
    }];
}
