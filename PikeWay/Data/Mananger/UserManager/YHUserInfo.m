...
- (void)setCompanyID:(NSString *)companyID{
    if (companyID) {
        ...
        if(_isSelfModel){
            [[NSUserDefaults standardUserDefaults] setObject:_companyID forKey:kEnterpriseId];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
    }
}
...
- (void)setAvatarImage:(UIImage *)avatarImage
{
	...
	if (!_avatarImage)
	{
		_avatarImage = [UIImage imageNamed:@"common_avatar_80px"];
	}
}
...
+ (NSDictionary *)yh_replacedKeyFromDictionaryWhenPropertyIsObject{
    return @{@"userSetting":[NSString stringWithFormat:@"userSetting%@",YHDB_AppendingID],
             @"companyInfo":[NSString stringWithFormat:@"companyInfo%@",YHDB_AppendingID]
             };
}

+ (NSDictionary *)yh_getClassForKeyIsObject{
    return @{@"userSetting":[YHUserSetting class],
             @"companyInfo":[YHCompanyInfo class]};
}
