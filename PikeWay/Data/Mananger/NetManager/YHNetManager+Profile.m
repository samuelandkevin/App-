...
@implementation YHNetManager (Profile)
...
- (void)postEditMyCardWithUserInfo:(YHUserInfo *)userInfo complete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathEditMyCard);
...
- (void)getMyCardDetailComplete:(NetManagerCallback)complete{
    NSString *requestUrl = RequestPath(kPathMyCard);
...
- (void)postModifyPasswd:(NSString *)newPasswd oldPasswd:(NSString *)oldPasswd complete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathModifyPasswd);
    ...
    NSString *newPasswdMD5 = [YHUtils md5HexDigest:newPasswd];
    NSString *oldPasswdMD5 = [YHUtils md5HexDigest:oldPasswd];
...
- (void)postValidateOldPasswd:(NSString *)oldPasswd complete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathValidateOldPasswd);
    
    //密码MD5加密
    NSString *passwdMD5 = [YHUtils md5HexDigest:oldPasswd];
...
- (void)postChangePhoneNum:(NSString *)newPhoneNum verifyCode:(NSString *)verifyCode complete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathChangePhone);
...
//    NSString *requestUrl = RequestPath(kPathGetMyVistors);
//
//    //参数判断
//    if(![YHUserInfoManager sharedInstance].userInfo.accessToken || ![YHUserInfoManager sharedInstance].userInfo.uid){
//        complete(NO,@"用户id 或 token 不能为空");
...
- (void)postUploadImage:(UIImage *)image complete:(NetManagerCallback)complete progress:(YHUploadProgress)progress{
    ...
    NSString *requestUrl = [NSString stringWithFormat:@"%@?accessToken=%@",kPathUploadImage,[YHUserInfoManager sharedInstance].userInfo.accessToken];
...
//上传图片(带图片来源)
- (void)postUploadImage:(UIImage *)image imgSource:(int)imgSource complete:(NetManagerCallback)complete progress:(YHUploadProgress)progress{
    ...
    if (imgSource == 0){
        complete(NO,@"图片来源为nil");
        return;
    }
    
    NSString *requestUrl = [NSString stringWithFormat:@"%@?accessToken=%@&imgSource=%@",kPathUploadImageWithSource,[YHUserInfoManager sharedInstance].userInfo.accessToken,@(imgSource)];
...
- (void)postChangeTaxAccount:(NSString *)taxAccount passwd:(NSString *)passwd complete:(NetManagerCallback)complete{
    ...
    NSString *passwdMD5 = [YHUtils md5HexDigest:passwd];
    ...
    NSString *requestUrl = RequestPath(kPathChangeTaxAccount);
...
- (void)getAppInfoComplete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathGetAppInfo);
...
- (void)postCheckUpdateComplete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathCheckUpdate);
    
    NSDictionary *params = @{
                             @"platform":iOSPlatform,
                             @"client_version":[YHUtils appStoreNumber],
                             kRequiedKeyAndValue
                             };
...
- (void)getIndustryListComplete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathIndustryList);
...
- (void)postEditJobTags:(NSArray *)jobTags complete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathEditJobTags);
...
- (void)postAddWorkExperience:(YHWorkExperienceModel*)workExperience complete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathEditWorkExp);
...
- (void)postAddEducationExperience:(YHEducationExperienceModel *)educationExperience complete:(NetManagerCallback)complete{
    ...
    NSString *requestUrl = RequestPath(kPathEditEducationExp);
