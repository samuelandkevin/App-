...
//工作指引目录
#define YHWorkGuideDir [YHUserDir stringByAppendingPathComponent:@"WorkGuide"]

//工作指引-继续阅读表路径
static inline NSString *pathConReading(){
    NSString *pathLog = [YHWorkGuideDir stringByAppendingPathComponent:[NSString stringWithFormat:@"conReading.sqlite"]];
    return pathLog;
}

//工作指引-继续阅读表表名
static inline NSString *tableNameConReading(){
    return [NSString stringWithFormat:@"conReading"];
}
